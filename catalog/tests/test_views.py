...
class AuthorCreateViewTest(TestCase):
    ...
    def test_login_with_correct_permission(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author_create'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_uses_initial_correctly(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author_create'))

        self.assertEqual(response.status_code, 200)
        
        date_initial = "05/01/2018"
        first_name_initial = "First Name"
      
        self.assertEqual(response.context['form'].initial['date_of_death'], "05/01/2018")
        self.assertEqual(response.context['form'].initial['first_name'], "First Name")
